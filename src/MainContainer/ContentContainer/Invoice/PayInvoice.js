...
const PayInvoice = props => {
  ...
  const invoice_id = props.id
  const payment = props.payment

  const { classes } = props
  const postTransactionMutation = useMutation(POST_TRANSACTION)
  const updateInvoicetMutation = useMutation(PUT_INVOICE_PAY)
...
export default withStyles(styles)(PayInvoice)
